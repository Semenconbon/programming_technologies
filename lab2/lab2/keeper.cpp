...
keeper::~keeper()
{
	cout << "¬ызван деструктор ~keeper" << endl;
}
...
void keeper::Save()
{
	...
	out.open("data.txt");  // окрываем файл дл€ записи
	...
	if (out.is_open())
	{
		for (int i = 0; i < size; i++) {
			...
			//фамили€;им€;знак зодиака;дата рождени€
...
void keeper::sort() {
	SIGN temp; // временна€ переменна€ дл€ обмена элементов местами

	// —ортировка массива пузырьком
	for (int i = 0; i < size - 1; i++) {
		for (int j = 0; j < size - i - 1; j++) {
			// проверка на год рождени€
...
void keeper::change_data(string surname) {
	for (int i = 0; i < size; i++) {
		if (sg[i].get_surname() == surname) {
			cout << sg[i] << endl;
			cout << "¬ведите его новые данные:" << endl;
			cout << "_______________________________" << endl;
			cin >> sg[i];
			
		}
		cout << "Ќовые данные:" << endl;
		cout << sg[i] << endl;
		break;
	}
}
